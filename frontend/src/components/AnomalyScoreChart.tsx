...
export function AnomalyScoreChart({ threshold = -0.1 }: { threshold?: number }) {
  const traffic = useEventStore(s => s.traffic);
  const hasScores = traffic.some(p => p.anomaly_score !== null);
  return <section className="panel"><div className="panel-heading"><div><h2>이상 점수 추이</h2><p>점수가 낮을수록 이상 가능성 증가</p></div><span className="subtle-label">임계값 {threshold}</span></div>
