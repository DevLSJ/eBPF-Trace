...
export function EventTable({ events, loading, onSelect }: { events: DetectionEvent[]; loading: boolean; onSelect: (id: number) => void }) {
  return <div className="table-wrap"><table><thead><tr><th>탐지 시간</th><th>출발지 → 목적지</th><th>탐지 유형</th><th>심각도</th><th>패킷 / s</th><th>이상 점수</th></tr></thead>
    <tbody>{events.map(event => <tr key={event.event_id}><td><span className="table-time">{new Date(event.detected_at).toLocaleTimeString('ko-KR', { hour12: false })}</span><small>{new Date(event.detected_at).toLocaleDateString('ko-KR')}</small></td>
      ...
      <td className="attack-type"><button className="event-link" aria-label={`이벤트 ${event.event_id} 상세 보기`} onClick={() => onSelect(event.event_id)}>{event.attack_type.replaceAll('_', ' ')}</button><small className={event.source === 'simulation' ? 'simulation-label' : ''}>{event.source === 'simulation' ? 'SIMULATION' : 'LIVE'} · {event.is_confirmed === true ? '정탐 확인' : event.is_confirmed === false ? '오탐 확인' : '검토 대기'}</small></td><td><SeverityBadge severity={event.severity}/></td><td className="number">{Math.round(event.features.pkt_rate).toLocaleString()}</td><td className="number">{event.anomaly_score?.toFixed(3) ?? '—'}</td></tr>)}</tbody></table>
