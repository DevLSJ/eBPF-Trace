...
export default function App() {
  ...
  const overviewView = !analysisView && !settingsView;
  const navigation = [
  ...
  useEffect(() => {
    const update = () => setSection(window.location.hash || '#overview');
    window.addEventListener('hashchange', update);
    return () => window.removeEventListener('hashchange', update);
  }, []);
  useEffect(() => {
    if (section === '#capture' || section === '#settings' || section === '#overview') window.scrollTo(0, 0);
    else document.querySelector(section === '#events' ? '#events' : '#system')?.scrollIntoView();
  }, [section]);
  ...
  useEffect(() => {
    ...
    getEvents({ page, page_size: 20, severity: severity || undefined, attack_type: attackType || undefined, ip: ip || undefined, start_time: since ? new Date(since).toISOString() : undefined }, controller.signal)
  ...
  return <div className="shell"><aside className="sidebar"><a className="brand" href="/"><div className="brand-mark"><Shield size={23}/></div><span>eBPF <b>Trace</b><small>NETWORK INTELLIGENCE</small></span></a>
    <div className="nav-caption">WORKSPACE</div><nav aria-label="주 메뉴">{navigation.map(({ href, label, icon: Icon }) => <a className={`nav-item ${section === href ? 'active' : ''}`} aria-current={section === href ? 'page' : undefined} href={href} key={href}><Icon size={17}/>{label}</a>)}</nav>
  ...
  </aside><div className="workspace"><header className="topbar"><div className="breadcrumb">Workspace <span>/</span><b>{analysisView ? 'Data analysis' : settingsView ? 'Detection settings' : 'Overview'}</b></div><div className="topbar-right"><ConnectionStatus/><span className="avatar">N</span></div></header>
    <nav className="mobile-nav" aria-label="모바일 메뉴">{navigation.filter(item => item.href !== '#system').map(item => <a href={item.href} key={item.href} className={section === item.href ? 'active' : ''} aria-current={section === item.href ? 'page' : undefined}>{item.label}</a>)}</nav>
    <main id="overview">{overviewView && <><div className="page-heading"><div><div className="eyebrow">NETWORK OBSERVABILITY</div><h1>네트워크 대시보드</h1><p>트래픽의 흐름부터 위협의 징후까지, 한눈에 확인하세요.</p></div><div className="date"><Globe2 size={15}/>{new Date(clock).toLocaleDateString('ko-KR', { year:'numeric', month:'long', day:'numeric' })}</div></div>
      <div className="summary-grid">{[
        { label: '저장된 탐지 이벤트', value: displayTotal.toLocaleString(), suffix: '건', icon: ShieldAlert, color: 'blue', note: severity || minutes || attackType || ip ? '현재 검색 조건 기준' : '전체 탐지 기록' },
      ...
      </section></>}{analysisView && <CapturePanel/>}{settingsView && <ThresholdSettings onSaved={value => setThreshold(value.anomaly_threshold)}/>}<footer>eBPF Trace <span>관찰 · 탐지 · 인사이트</span><span className="footer-right">모든 패킷은 정상 통과됩니다.</span></footer>
