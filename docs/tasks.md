...
아래 9월 17일의 사건 대응 ‘설계’ 상태는 [이번 구현 기록](incident-operations.md)으로 갱신한다. 운영 배포와 실제 채널 활성화는 수행하지 않았다.

- 구현: 개인 계정·RBAC·사건 통합·내구성 알림·담당자 인수/인계·2인 승인·기한부 nftables 에이전트·해제/복구 검증·JSON 보고서.
- UI: 사건 받은편지함, 단계별 다음 행동, 증거/대응/타임라인, 승인함·알림·자산·모델 운영, 모바일 배치.
- ML: 해시·스키마 검증 Shadow runner, 승격 게이트, 판정 근거, 추론 오류 자동 폴백. 새 독립 성능 검증/Production 승격을 의미하지 않는다.
- 검증: Python 96개(전용 PostgreSQL·Redis 포함), 브라우저 20개 범위, 격리 Linux 커널의 실제 차단/정상 연결 보존/자동 만료/해제, Ruff·TypeScript·Vite.
- 운영 대기: HTTPS·개인 계정 발급·Slack/SMTP 실제 전달, 현장 서비스 probe, 독립 ML 검증과 LIVE Shadow 7일, 승인된 배포.

## 2026-09-17 현재 상태

이 섹션이 아래 과거 기록과 초기 Phase 체크리스트보다 우선한다. 상세 근거: [전체 요일 검증](verification-2026-09-17-context.md).

| 상태 | 범위 | 결과 |
|---|---|---|
| 완료 | 5일 PCAP·정답 결합 | 52.43 GB, 15,226,375개 피처 구간 중 12,250,475개 매칭; 월요일 초 정밀도·오후 시각 수정 |
| 완료 | 문맥 피처·성능 최적화 | 목적지/서비스/양방향 등 25개 공유 피처, 증분 집계·해시 기반 재개·학습 표본 상한 |
| 완료 | 모델 비교 | 5개 후보; 보정으로 선택한 HGB 시험 F1 90.98%, FPR 0.26%; 시험은 공격 4/14종만 포함 |
| 완료 | 백엔드·DB | v2 완전성 검증·구버전 호환·이벤트 JSON 보존·분석 보고서 캐시·운영 관측 스키마 표시 |
| 완료 | 프론트엔드 | 5일 매칭 현황·모델 선택·시험 범위·유형별 재현율·보고서 다운로드·이벤트 문맥 상세 |
| 완료 | 시나리오·검토 | 실행 ID로 DB 그래프/이벤트 연결, 모의/실시간 분리, 관리자 정탐·오탐 검토 |
| 완료 | 로컬 검증 | 69개 Python 테스트 범위 검증(실 PostgreSQL/Redis 포함), 브라우저 16개, TypeScript/Vite/Ruff |
| 완료 | 웹/API 배포 | main `bde12ed`; CI Python 69개·브라우저 16개·eBPF 컴파일·이미지 빌드·EC2 배포 성공; 운영 화면 10개 캡처 |
| 대기 | VM Collector 갱신 | 기존 Collector 연결/스키마 1 유지; runner의 서비스 시작·중지 sudo 권한 설정 후 배포 job 재실행 필요 |
| 설계 | 사건 대응 | Slack/Gmail 전달 신뢰성·담당자 인수·승인형 기한부 대응 설계; 발송/자동 차단 미활성화 |
| 후속 | 모델 운영 승인 | 시험에서 빠진 10종·Bot 강화, 신규 공격 실행·환경의 독립 검증 |
| 후속 | 운영 확장 | HTTPS, 알림 워크플로우 구현, Terraform 기존 자원 정합 |

운영은 규칙 기반이며, 이번 실험 모델은 배포하지 않는다. 원본 `label/`, `pcap/`, 생성 피처·모델은 Git/Docker에서 제외한다. [대응 기획](response-design-2026-09-17.md) · [이전 목·금요일 실험](verification-2026-09-17-upgrade.md).
