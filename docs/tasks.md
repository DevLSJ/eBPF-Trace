...
- [ ] 0-2. GitHub 레포지토리 초기화 및 `.gitignore` 설정
  - `.env`, `*.pkl`, `terraform.tfvars`, `__pycache__/` 등 제외
- [ ] 0-3. 공통 `.env.example` 파일 작성
  - 전체 환경 변수 키 목록 정의 (값 없이 키만)
- [ ] 0-4. Linux VM 개발 환경 구성
...
- [ ] 0-5. 백엔드 Python 가상환경 구성
  - Python 3.11+, `venv` 생성, `requirements.txt` 초안 작성
- [ ] 0-6. 프론트엔드 Node 환경 구성
  - Node.js 20 LTS, Vite + React + TypeScript 프로젝트 생성
- [ ] 0-7. 개발용 Docker Compose 구성 (`docker-compose.dev.yml`)
  - PostgreSQL 16, Redis 7 로컬 컨테이너 기동 확인
